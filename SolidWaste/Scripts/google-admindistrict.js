...
var map,
    ...
    function placeUserMarker(location, firstName, lastName, Address, id, routeid, status) {
		...
		if(status == 'For Verification') {
		    ...
		    div.appendChild(helper.createEl('button', 'btn btn-secondary', null, 'Reject', function () { btnfuncs.rejectUserForm(this, id); }));
		}else if(status == 'Rejected') {
		    div.appendChild(helper.createEl('p', null, null, 'Awaiting response from rejection'));
		}
		if(routeid != null) {
		...
		} else if (status == 'For Verification' || status == 'Rejected') { // user account is not verified
    ...
    $(document).ready(function() {
        ...
        $('#btn-reject-user').click(function () {
            btnfuncs.rejectUser($(this).data('userid'));
        });
    ...
    var btnfuncs = {
		...
		rejectUserForm: function (btnEl, userID) {
		    $('#btn-reject-user').data('userid', userID);
			temp = btnEl;
		...
		rejectUser: function (userid) {
			...
			var rejectMessage = $('#rejectMessage').val();
			var btnEl = temp;
            ...
            $.ajax(options).done(function (data) {
				...
				parent.append('<p>Awaiting response from rejection</p>');
		...
		updateRouteForm: function (assignedDriverID, assignedTruckID, RouteID) {
			...
			$.ajax(options).done(function (data) {
				...
				tr3.appendChild(helper.createEl('td', null, null, 'Assigned Day: '));
